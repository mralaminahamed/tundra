...
mod tests {
    ...
    async fn reconcile_ok() {
        let spec = RoundcubeSpec {
            mail_domain: "example.com".into(),
            webmail_domain: "webmail.example.com".into(),
            imap_host: "ssl://localhost:993".into(),
            smtp_host: "tls://localhost:587".into(),
            db_dsn: "host=127.0.0.1 dbname=roundcube".into(),
            des_key: "rcmTypeK3yFOrTestOnly!!1".into(),
        };
        assert_eq!(
            RoundcubeProvider.reconcile(&spec).await.unwrap(),
