...
function AvailableCard({ p, onInstall, installing }: {
  ...
  return (
    <div className={`relative flex flex-col rounded-xl border bg-white p-5 transition-shadow hover:shadow-md ${
      ...
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="flex items-center gap-2.5">
          <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-sm font-bold ${
            p.official ? 'bg-tundra-lichen-100 text-tundra-lichen-700' : 'bg-tundra-ink-100 text-tundra-ink-500'
          }`}>
            {initials}
          </div>
          <div>
            <h3 className="text-sm font-semibold text-tundra-ink leading-tight">{p.name}</h3>
            <p className="font-mono text-xs text-tundra-ink-400">{p.plugin_id}</p>
          </div>
        </div>
        {p.installed ? (
          <span className="inline-flex items-center gap-1.5 rounded-full border border-tundra-lichen-300 bg-tundra-lichen-50 px-2.5 py-0.5 text-xs font-medium text-tundra-lichen-800">
...
function InstalledCard({ p, onEnable, onDisable, acting }: {
  ...
  const initials = p.manifest.name.split(' ').slice(0, 2).map((w) => w[0]).join('').toUpperCase()
  return (
    <div className={`relative flex flex-col rounded-xl border bg-white p-5 transition-shadow hover:shadow-md ${
      ...
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="flex items-center gap-2.5">
          <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-sm font-bold ${
            p.state === 'enabled' ? 'bg-tundra-lichen-100 text-tundra-lichen-700'
            : p.state === 'quarantined' ? 'bg-red-100 text-red-600'
            : 'bg-tundra-ink-100 text-tundra-ink-500'
          }`}>
            {initials}
          </div>
          <div>
            <h3 className="text-sm font-semibold text-tundra-ink leading-tight">{p.manifest.name}</h3>
            <p className="font-mono text-xs text-tundra-ink-400">{p.plugin_id}</p>
          </div>
        </div>
