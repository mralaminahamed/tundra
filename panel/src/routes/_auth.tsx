...
const NAV_GROUPS: {
  label: string
  items: { to: string; label: string; icon: IconKey; exact?: boolean }[]
}[] = [
  {
    label: 'Overview',
    items: [
...
function AuthLayout() {
  ...
  const navGroups = NAV_GROUPS.map((group) => {
    ...
    const wpItem: { to: string; label: string; icon: IconKey; exact?: boolean } = { to: '/wordpress', label: 'WordPress', icon: 'wordpress' }
  ...
  return (
    <div className="flex min-h-screen bg-tundra-paper">
      {/* Sidebar */}
      <aside
        className={`${collapsed ? 'w-14' : 'w-60'} shrink-0 border-r border-tundra-ink-200 bg-white flex flex-col sticky top-0 h-screen transition-[width] duration-200`}
      >
        ...
        {/* Footer */}
        <div className={`border-t border-tundra-ink-100 ${collapsed ? 'flex flex-col items-center gap-1 py-3 px-2' : 'p-3'}`}>
          {/* Profile card */}
          <Link
            to="/settings/profile"
            title={collapsed ? (operator?.full_name ?? operator?.email ?? 'My profile') : undefined}
            activeOptions={{ exact: true }}
            className={`flex items-center gap-2.5 rounded-lg transition-colors ${collapsed ? 'justify-center p-2 w-full' : 'w-full px-2 py-2'}`}
            activeProps={{ className: 'bg-tundra-lichen-50 text-tundra-lichen' }}
            inactiveProps={{ className: 'text-tundra-ink hover:bg-tundra-ink-50' }}
          >
            {/* Avatar */}
            <div className="h-7 w-7 shrink-0 rounded-full bg-tundra-lichen-100 flex items-center justify-center text-[10px] font-bold text-tundra-lichen-800 select-none">
              {(operator?.full_name ?? operator?.email ?? '?')
                .split(' ').map((w) => w[0]).join('').slice(0, 2).toUpperCase()}
            </div>
            {!collapsed && (
              <div className="flex-1 min-w-0">
                <div className="text-xs font-medium text-tundra-ink truncate leading-tight">
                  {operator?.full_name || operator?.email}
                </div>
                <div className="text-[10px] text-tundra-ink-400 truncate leading-tight">
                  {operator?.email}
                </div>
              </div>
            )}
          ...
          <button
            ...
            className={`flex items-center gap-2 rounded-md text-tundra-ink-400 hover:text-tundra-rust hover:bg-tundra-rust-50 transition-colors text-xs ${collapsed ? 'justify-center p-2 w-full' : 'w-full px-2 py-1.5'}`}
          >
